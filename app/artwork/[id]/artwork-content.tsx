...
export default function ArtworkContent({ initialSession, artwork }: ArtworkContentProps) {
  const router = useRouter()
  const isArtist = initialSession?.user?.id === artwork.artist.id
  ...
  const handleEdit = async () => {
    const validation = validateArtwork({
      ...
      category: editedArtwork.category.id
    })
    ...
    try {
      const response = await fetch(`/api/artworks/${artwork.id}`, {
        ...
        body: JSON.stringify({
          ...editedArtwork,
          categoryId: editedArtwork.category.id
        }),
      })
